...
using GroupDocs.Conversion.Mcp.Tools;
using Microsoft.Extensions.Options;
using Moq;
using Xunit;

namespace GroupDocs.Conversion.Mcp.Tests;

public class GetSupportedFormatsToolTests
{
    ...
    public GetSupportedFormatsToolTests()
    {
        _output = new OutputHelper(_storage.Object, Microsoft.Extensions.Options.Options.Create(new McpConfig()));
    }

    [Fact]
    public async Task GetSupportedFormats_WhenResolverThrows_PropagatesException()
    {
        _resolver
            .Setup(r => r.ResolveAsync(It.IsAny<FileInput>(), It.IsAny<CancellationToken>()))
            .ThrowsAsync(new FileNotFoundException("missing.docx"));

        var ex = await Assert.ThrowsAsync<FileNotFoundException>(() =>
            GetSupportedFormatsTool.GetSupportedFormats(
                _resolver.Object,
                _licenseManager.Object,
                _output,
                new FileInput { FilePath = "missing.docx" }));

        Assert.Contains("missing.docx", ex.Message);
    }

    [Fact]
    public async Task GetSupportedFormats_SetsLicense_BeforeResolving()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            GetSupportedFormatsTool.GetSupportedFormats(
                ...
                _licenseManager.Object,
                _output,
                new FileInput { FilePath = "anything.docx" }));

        Assert.Equal(new[] { "license", "resolve" }, sequence);
    }

    [Fact]
    public async Task GetSupportedFormats_PassesFileInputToResolver_Unchanged()
    {
